...
7. Коммиты: conventional-commit (`feat:`, `fix:`, `docs:`, `style:`...) с gitmoji-шорткодом,
   сообщения на английском. Не коммитить `.opencode/continue/` и `.playwright-mcp/`.
   Не пушить без явной просьбы владельца.

## Скиллы (маршрутизация)

| Skill | Когда |
|---|---|
| `continue` | любая многошаговая задача; стейт в `.opencode/continue/<task-id>.md` |
| `nodumb` | перед нетривиальной развилкой, массовой правкой, редизайном |
| `karpathy-guidelines` | при написании/ревью любого кода |
| `stop-slop` | при правке текстов контента и документации |
| `anti-ui-slop`, `ui-design`, `ui-radar` | UI-задачи T02–T10; референс ziit.app |
| `conventional-commit` | подготовка текстов коммитов |
| `/review`, `/commit` | шорткаты: ревью диффа, сборка текстов коммитов |

## Definition of Done

- Сценарий работает end-to-end в браузере (dev или preview), а не «код написан».
- `npm run build` + `npm run check` зелёные — с фактическими статусами.
- UI-правки проверены скриншотами 375px и 1280px через playwright MCP.
- Тексты обеих локалей согласованы; риски и непроверенное названы явно.
